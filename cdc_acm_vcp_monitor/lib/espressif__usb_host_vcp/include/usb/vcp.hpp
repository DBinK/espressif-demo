 ...
 */
 using open_func_t = CdcAcmDevice* (*)(uint16_t pid, const cdc_acm_host_device_config_t *dev_config, uint8_t interface_idx);
class VCP {
public:
    /**
    ...
    static void register_driver(open_func_t openFunc, const std::vector<uint16_t>& pids, const uint16_t vid)
    {
        vcp_driver new_driver = vcp_driver(openFunc, vid, pids);
...
private:
    ...
    typedef struct vcp_driver {
        ...
        vcp_driver(open_func_t open_func, const uint16_t _vid, const std::vector<uint16_t> &_pids): open(open_func), vid(_vid), pids(_pids) {};
    } vcp_driver;
